    ...
    <div className='animate-fade-in'>
      ...
      <div className='flex flex-col sm:flex-row gap-3 mb-6'>
        <div className='relative flex-1'>
          ...
          <input className='w-full pl-10!' type="text" placeholder='Search Teachers...'
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5'>
            {
              filtered.length === 0 ? (
                ...
                filtered.map((f) => (
                  <TeacherCard key={f._id} teacher={f} onDelete={fetchTeachers} onEdit={(e) => setEditTeacher(e)} />
                ))
              )
            }
          </div>
            ...
            w-full max-w-3xl my-8 animate-fade-in'>
              <div className='flex items-center justify-between p-6 pb-0'>
                <div>
                  ...
                  <p className='text-sm text-slate-500 mt-0.5'>Create a teacher account and profile</p>
                </div>
        ...
        editTeacher && (
          ...
          justify-center p-4 overflow-y-auto'>
            ...
            w-full max-w-3xl my-8 animate-fade-in'>
              <div className='flex items-center justify-between p-6 pb-0'>
                <div>
                  <h2 className='text-lg font-semibold text-slate-900'>Edit Teacher</h2>
                  <p className='text-sm text-slate-500 mt-0.5'>Update reacher details</p>
                </div>
                <button onClick={() => setEditTeacher(null)} className='p-2 rounded-lg hover:bg-slate-100 transition-colors
                text-slate-400 hover:text-slate-600'>
                  <X className='w-5 h-5' />
                </button>
              </div>
              <div className='p-6'>
                <TeacherForm initialData={editTeacher} 
                onSuccess={() => {
                  setEditTeacher(null)
                  fetchTeachers()
                }} 
