  ...
  return (
    <div className='animate-fade-in'>
      ...
      <div className='space-y-5'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
              {
                filteredClasses.length === 0 ? (
                  ...
                  filteredClasses.map((classItem) => (
                    <ClassCard key={classItem.name} classes={classItem} onDelete={fetchClasses} onEdit={setEditClasses} />
                  ))
                )
              }
            </div>
            ...
            w-full max-w-3xl my-8 animate-fade-in'>
                <div className='flex items-center justify-between p-6 pb-0'>
                  <div>
                    ...
                    <p className='text-sm text-slate-500 mt-0.5'>Create a Class with grade level and allocated subjects</p>
                  </div>
        ...
        {
          editClasses && (
            <div onClick={() => setEditClasses(null)} className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start
          justify-center p-4 overflow-y-auto'>
              <div onClick={(e) => e.stopPropagation()} className='relative bg-white rounded-2xl shadow-2xl 
            w-full max-w-3xl my-8 animate-fade-in'>
                <div className='flex items-center justify-between p-6 pb-0'>
                  ...
                  <button onClick={() => setEditClasses(null)} className='p-2 rounded-lg hover:bg-slate-100 transition-colors
                text-slate-400 hover:text-slate-600'>
                    <X className='w-5 h-5' />
                  </button>
                </div>
                <div className='p-6'>
                  <ClassForm initialData={editClasses}
                    onSuccess={() => {
                      setEditClasses(null)
                      fetchClasses()
                    }}
