...
import { Plus, X, PencilIcon, Trash2Icon, CalendarIcon, MapPinIcon, UsersIcon } from 'lucide-react'
import {
  dummyEventData, EVENT_TYPES, EVENT_AUDIENCE, EVENT_STATUSES,
  getEventStatusDisplay, formatDate
  ...
  return (
    <div className='animate-fade-in'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8'>
        <div>
          ...
          <p className='page-subtitle'>
            {
              isAdmin ? "Create and manage school events" : "View upcoming and ongoing school events"
            }
          </p>
        </div>
        {
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
          {filteredEvents.map((event) => {
            const { label, className: statusClass } = getEventStatusDisplay(event.status)
            return (
              <div key={event._id} className='card p-5 flex flex-col gap-3'>
                {/* Card header */}
                <div className='flex items-start justify-between gap-2'>
                  <div className='flex-1'>
                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs border mb-2 ${statusClass}`}>
                      {label}
                    </span>
                    <h3 className='font-medium text-slate-800 leading-snug'>{event.title}</h3>
                  </div>
                  {
                    isAdmin && (
                ...
                <div className='space-y-1.5 mt-auto pt-3 border-t border-slate-100'>
                  ...
                  <div className='flex items-center gap-2 text-xs text-slate-500'>
                    ...
                    <span>{event.audience} · {event.type}</span>
                  </div>
                </div>
              </div>
            )
      ...
      {showCreateModal && modalShell(
        ...
        <EventForm
          onSuccess={() => { setShowCreateModal(false); fetchEvents() }}
      ...
      {/* Edit Modal */}
      {editEvent && modalShell(
        'Edit Event',
        'Update event details',
        <EventForm
          initialData={editEvent}
          onSuccess={() => { setEditEvent(null); fetchEvents() }}
      ...
      {deleteTarget && (
        <div onClick={() => setDeleteTarget(null)} className='fixed bg-black/40 backdrop-blur-sm
                inset-0 z-50 flex items-center justify-center p-4'>
          <div onClick={(e) => e.stopPropagation()} className='relative bg-white rounded-2xl
                    shadow-2xl w-full max-w-sm p-6 animate-fade-in'>
            <h2 className='text-lg font-semibold text-slate-900 mb-1'>Delete Event</h2>
