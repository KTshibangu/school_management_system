  ...
  return (
    <div className='animate-fade-in'>
      ...
      <div className='space-y-5'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
              {
                filteredSubjects.length === 0 ? (
                  <p className='text-sm text-slate-400 col-span-full'>No subjects found for this grade.</p>
                ) : (
                  filteredSubjects.map((subject) => (
                    <SubjectCard key={subject.code} subject={subject} onDelete={fetchSubjects} onEdit={setEditSubject} />
                  ))
                )
              }
            </div>
          ...
          justify-center p-4 overflow-y-auto'>
            ...
            w-full max-w-3xl my-8 animate-fade-in'>
                <div className='flex items-center justify-between p-6 pb-0'>
                  <div>
                    <h2 className='text-lg font-semibold text-slate-900'>Add New Subject</h2>
                    <p className='text-sm text-slate-500 mt-0.5'>Create a subject with its code and grade level</p>
                  </div>
        ...
        {
          editSubject && (
            <div onClick={() => setEditSubject(null)} className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start
          justify-center p-4 overflow-y-auto'>
              <div onClick={(e) => e.stopPropagation()} className='relative bg-white rounded-2xl shadow-2xl 
            w-full max-w-3xl my-8 animate-fade-in'>
                <div className='flex items-center justify-between p-6 pb-0'>
                  <div>
                    <h2 className='text-lg font-semibold text-slate-900'>Edit Subject</h2>
                    <p className='text-sm text-slate-500 mt-0.5'>Update Subject details</p>
                  </div>
                  <button onClick={() => setEditSubject(null)} className='p-2 rounded-lg hover:bg-slate-100 transition-colors
                text-slate-400 hover:text-slate-600'>
                    <X className='w-5 h-5' />
                  </button>
                </div>
                <div className='p-6'>
                  <SubjectForm initialData={editSubject}
                    onSuccess={() => {
                      setEditSubject(null)
                      fetchSubjects()
                    }}
