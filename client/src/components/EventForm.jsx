...
import { EVENT_TYPES, EVENT_AUDIENCE, EVENT_STATUSES } from '../assets/myassets'
...
const EventForm = ({ initialData, onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const data = { title, description, type, audience, status, location, startDate, endDate }
    ...
    return (
        <form onSubmit={handleSubmit} className='space-y-5 animate-fade-in'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-5'>
                <div>
                    <label className='block text-sm text-slate-600 mb-1.5'>Start Date & Time</label>
                    <input
                ...
                <div>
                    <label className='block text-sm text-slate-600 mb-1.5'>End Date & Time</label>
                    <input
                        type='datetime-local' required
                        value={endDate}
